...
backend appserver {
    # private ip for prod.app1.iucn.edw.ro server
    .host = "10.0.1.5";
    .port = "80";
...
acl purge {
    "116.203.75.26";   # prod.app1.iucn.edw.ro server IP
    "116.203.166.248"; # prod.app1.iucn.edw.ro floating IP
    "82.76.35.69";     # EdW Public IP
    "109.103.137";     # EdW Public IP
    "95.217.24.77";    # EdW VPN Hetzner
}

sub get_real_ip {
    # Verify if `X-Real-IP` is set, otherwise use `client.ip`
    if (req.http.X-Real-IP) {
        set req.http.real_ip = req.http.X-Real-IP;
    } else {
        set req.http.real_ip = client.ip;
    }
}
...
sub vcl_recv {
    ...
    if (req.method == "PURGE") {
        # Check if the client REAL IP is allowed from the purge list.
        if (!std.ip(req.http.real_ip, client.ip) ~ purge) {
